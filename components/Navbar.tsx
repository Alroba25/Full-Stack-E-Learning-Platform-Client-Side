      ...
      <div className="hidden md:flex gap-10">
        {navLinks.map((link) => {
          ...
          return (
            ...
            >
              <span className={`font-medium text-[0.95rem] transition-colors ${isActive ? "text-white" : "text-[#a0a0a0] group-hover:text-white"}`}>
                {link.name}
              </span>
              <span className={`absolute bottom-0 left-0 h-0.5 bg-linear-to-r from-[#00f2fe] to-[#4facfe] transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"}`}></span>
            </Link>
          );
        })}
        ...
        {token && (
          <Link
            ...
            className="relative group flex items-center py-1"
          >
            <span className={`font-bold text-[0.95rem] transition-colors ${pathname === "/my-courses" ? "text-[#4facfe]" : "text-white group-hover:text-[#4facfe]"}`}>
              My Courses
            </span>
            <span className={`absolute bottom-0 left-0 h-0.5 bg-linear-to-r from-[#00f2fe] to-[#4facfe] transition-all duration-300 ${pathname === "/my-courses" ? "w-full" : "w-0 group-hover:w-full"}`}></span>
          </Link>
        )}
      </div>
